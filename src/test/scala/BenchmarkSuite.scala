...
extends AnyFunSpec with CancelAfterFailure with SqlTestRunner {
    ...
    describe("JDBC driver") {
        it("should setup") {
            ...
            // stmt.executeUpdate("explain script")
        }

        it("should add postgresql location") {
            stmt.executeUpdate(
                "add location scleratest as postgresql('localhost/scleratest')"
            )
        }
        ...
        it("should create a test table") {
            stmt.executeUpdate("create table T(col text) as values ('bar')")
        }
        ...
        it("should remove postgresql location") {
